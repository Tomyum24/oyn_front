...
function MonthPicker({
  ...
  return (
    <div className={cn("w-fit p-3", className)} {...props}>
      ...
      <table className="w-full border-collapse space-y-1">
        <tbody>
          {MONTHS.map((monthRow, a) => {
            return (
              <tr key={"row-" + a} className="mt-2 flex w-full gap-x-2">
                {monthRow.map((m) => {
                  return (
                    <td
                      key={m.number}
                      className="[&:has([aria-selected])]:bg-accent [&:has([aria-selected].day-outside)]:bg-accent/50 relative h-10 w-10 p-0 text-center text-sm focus-within:relative focus-within:z-20 first:[&:has([aria-selected])]:rounded-l-md last:[&:has([aria-selected])]:rounded-r-md [&:has([aria-selected].day-range-end)]:rounded-r-md"
                    >
                      <button
                        onClick={() => {
                          setMonth(m.number);
